...
export class Validator<T, K extends keyof T> {
    ...
    withErrors(errs: ErrorMessage<T>[]): Validator<T, K> {
        if (errs.length) {
    ...
    when(condition: boolean | undefined | ValidationCallback<T, K>, callback: ValidationCallback<T, K>): Validator<T, K> {
        ...
        if (typeof condition === 'function') {
            ...
            // if there were errors generated by the condition function, merge them in.
            this.withErrors(errs);
            
            // If no errors, run the passed-in callback
            if (errs.length === 0) {
                this.run(callback);
            }
